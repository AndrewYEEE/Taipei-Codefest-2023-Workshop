...
def get_traveltaipei_api(APIPath):
    '''
    ...
    url = f"""https://www.travel.taipei/open-api/zh-tw/{APIPath}?page=1"""
    print(url)
    ...
    res = list()
    offset_count = int(count/30)
    for i in range(offset_count):
        i = i+1
        url = f"""https://www.travel.taipei/open-api/zh-tw/{APIPath}?page={i}"""
        print(url)
        response = requests.request("GET", url,headers=headers)
        get_json = response.json()
        res.extend(get_json['data'])
        if i > 1 : #記得要刪掉
            break
    return pd.DataFrame(res)
